...
docker buildx build \
  ...
  -t tonovarela/notificacion-clientes:1.0.5 \
...
docker buildx build \
  ...
  -t tonovarela/notificacion-clientes:1.0.5-prueba \
  --push .
```
...
la versión fija (`:1.0.5`), nunca a `:latest`.
...
```bash
docker run --rm --entrypoint sh tonovarela/notificacion-clientes:1.0.5 -c '
